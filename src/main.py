                ...
                if ignore_next == 0:
                    ...
                    if not first_hold: #locked piece
                        if session.piece_count >= 0:
                            ...
                            #if board_updated < session.piece_count: # try to update the board
                            #    prev_candidates = temp_playfield.reconstruct_board_first_call(defaults['pieces'], last_playfield, prev_candidates)
                            #    print('Matches:', prev_candidates)

                                # if len(matches) == 1:
                                #     for i in range(len(matches[0])):
                                #         board_diff = update_board(defaults, matches[0][i], last_playfield)
                                #         board_updated += 1
                                #         piece_id = matches[0][i][0]
                                #         print(piece_id)
                                #         temp_playfield.update_with(board_diff, piece_id, 0)
                                #         playfield.update_with(board_diff, piece_id, 0)
                                #         board_id.update_with(board_diff, board_updated, session.total_lines)
                                        #print(board_id.board)

                                #         last_playfield = copy.deepcopy(temp_playfield)
                                #         prev_candidates.pop()
                                # length = 1
                                # while length == 1 and len(prev_candidates) > 0:
                                #     piece_candidates = prev_candidates[0]
                                #     piece_id = piece_candidates[0][0]
                                    
                                #     candidates = temp_playfield.reconstruct_board_from_candidates(defaults['pieces'], last_playfield, piece_candidates)
                                #     length = len(candidates)
                                #     print('Trying to update...', piece_candidates, length)
                                    
                                #     if length == 1:
                                #         board_diff = update_board(defaults, candidates, last_playfield)
                                #         #print(board_diff)
                                #         board_updated += 1
                                #         temp_playfield.update_with(board_diff, piece_id, 0)
                                #         playfield.update_with(board_diff, piece_id, 0)
